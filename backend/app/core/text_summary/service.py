"""Text summarization service - orchestration logic."""
import os
import uuid
...
class TextSummaryService:
    """
    Service for text summarization using RAG pipeline.
    ...
    async def summarize(
        ...
        """
        Summarize content about a topic/query with streaming response.
    ...
    # Note: Session management methods (get_sessions, get_session_messages, delete_session)
    # have been moved to /api/sessions.py as universal endpoints for all tasks.
    # These methods are kept here for backward compatibility but are no longer used.


# Singleton instance
_text_summary_service = None

def get_text_summary_service() -> TextSummaryService:
    """Get singleton text summary service."""
    global _text_summary_service
    if _text_summary_service is None:
        _text_summary_service = TextSummaryService()
    return _text_summary_service
